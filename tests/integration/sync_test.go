...
var (
	...
	consumerGroupScopedPlugins313x = []*kong.Plugin{
		{
			Name: kong.String("rate-limiting-advanced"),
			ConsumerGroup: &kong.ConsumerGroup{
				ID: kong.String("77e6691d-67c0-446a-9401-27be2b141aae"),
	...
	consumerGroupScopedPlugins315x = []*kong.Plugin{
		{
			Name: kong.String("rate-limiting-advanced"),
			ConsumerGroup: &kong.ConsumerGroup{
				ID: kong.String("77e6691d-67c0-446a-9401-27be2b141aae"),
...
func Test_Sync_ServicesRoutes_From_3x(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			runWhen: ">=3.0.0 <3.14.0",
		},
		{
			name:     "create services and routes 3.14+",
			kongFile: "testdata/sync/002-create-services-and-routes/kong3x.yaml",
			expectedState: utils.KongRawState{
				Services: svc1_207,
				Routes:   route314x,
...
func Test_Sync_Upstream_Target_From_3x(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			runWhen: ">=3.1.0 <3.11.0",
		},
		{
			name:     "creates an upstream and target >=3.11.0",
			kongFile: "testdata/sync/004-create-upstream-and-target/kong3x.yaml",
			expectedState: utils.KongRawState{
				Upstreams: upstream,
				Targets:   target,
...
func Test_Sync_Upstreams_Target_ZeroWeight_3x(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "creates an upstream and target with weight equals to zero >=3.11.0 ",
			kongFile: "testdata/sync/005-create-upstream-and-target-weight/kong3x.yaml",
			expectedState: utils.KongRawState{
				Upstreams: upstream,
				Targets:   targetZeroWeight,
...
func Test_Sync_Vault(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			runWhen: ">=3.11.0 <3.14.0",
		},
		{
			name:     "create an SSL service/route using an ENV vault >=3.14.0",
			kongFile: "testdata/sync/012-vaults/kong3x.yaml",
			expectedState: utils.KongRawState{
				Vaults: []*kong.Vault{
					{
	...
	tests := []struct {
		...
		},
		{
			...
			expectedState: utils.KongRawState{
				...
				Plugins:  consumerGroupScopedPlugins315x,
				Services: svc1_207,
				Routes:   route1_20x,
				KeyAuths: []*kong.KeyAuth{
					{
...
func Test_Sync_GraphqlRateLimitingCostDecorations(t *testing.T) {
	...
	ctx := context.Background()
	dumpConfig := deckDump.Config{CustomEntityTypes: []string{"graphql_ratelimiting_cost_decorations", "degraphql_routes"}}

	tests := []struct {
		name          string
		kongFile      string
		errorExpected string
		validateFunc  func(t *testing.T, newState *state.KongState)
	}{
		{
			name:     "create graphql ratelimiting cost decoration",
			kongFile: "testdata/sync/052-graphql-ratelimiting-cost-decorations/kong.yaml",
			validateFunc: func(t *testing.T, newState *state.KongState) {
		...
		},
		{
			...
			kongFile: "testdata/sync/052-graphql-ratelimiting-cost-decorations/kong-all-fields.yaml",
			validateFunc: func(t *testing.T, newState *state.KongState) {
				decorations, err := newState.GraphqlRateLimitingCostDecorations.GetAll()
				require.NoError(t, err)
		...
		},
		{
			...
			kongFile:      "testdata/sync/052-graphql-ratelimiting-cost-decorations/kong-missing-type-path.yaml",
		...
		},
		{
			...
			kongFile: "testdata/sync/052-graphql-ratelimiting-cost-decorations/kong-mixed.yaml",
			validateFunc: func(t *testing.T, newState *state.KongState) {
				decorations, err := newState.GraphqlRateLimitingCostDecorations.GetAll()
				require.NoError(t, err)
				require.Len(t, decorations, 1)
...
func Test_Sync_EnvVar_Masking(t *testing.T) {
	...
	t.Run("certificate rotation with masked output", func(t *testing.T) {
		...
		t.Setenv("DECK_CLIENT_CERT", certA)
		t.Setenv("DECK_CLIENT_KEY", keyA)
	...
	t.Run("jwk key with masked output", func(t *testing.T) {
		reset(t)
		jwkKey := `{"kid":"vsR8NCNV_1_LB06LqudGa2r-T0y4Z6VQVYue9IQz6A4","kty":"RSA","alg":"A256GCM","n":"v2KAzzfruqctVHaE9WSCWIg1xAhMwxTIK-i56WNqPtpWBo9AqxcVea8NyVctEjUNq_mix5CklNy3ru7ARh7rBG_LU65fzs4fY_uYalul3QZSnr61Gj-cTUB3Gy4PhA63yXCbYRR3gDy6WR_wfis1MS61j0R_AjgXuVufmmC0F7R9qSWfR8ft0CbQgemEHY3ddKeW7T7fKv1jnRwYAkl5B_xtvxRFIYT-uR9NNftixNpUIW7q8qvOH7D9icXOg4_wIVxTRe5QiRYwEFoUbV1V9bFtu5FLal0vZnLaWwg5tA6enhzBpxJNdrS0v1RcPpyeNP-9r3cUDGmeftwz9v95UQ","e":"AQAB"}`
		t.Setenv("DECK_JWK_KEY", jwkKey)

		output, err := syncWithOutput(context.Background(), "testdata/sync/057-env-var-masking-tests/jwk-key.yaml")
		require.NoError(t, err)

		jwkKey2 := `{"kid":"vsR8NCNV_1_LB06LqudGa2r-T0y4Z6VQVYue9IQz6A4","kty":"RSA","alg":"A256GCM","n":"v2KAzzfruqcrVHaE9WSCWIg1xAhMwxTIK-i56WNqPtpWBo9AqxcVea8NyVctEjUNq_mix5CklNy3ru7ARh7rBG_LU65fzs4fY_uYalul3QZSnr61Gj-cTUB3Gy4PhA63yXCbYRR3gDy6WR_wfis1MS61j0R_AjgXuVufmmC0F7R9qSWfR8ft0CbQgemEHY3ddKeW7T7fKv1jnRwYAkl5B_xtvxRFIYT-uR9NNftixNpUIW7q8qvOH7D9icXOg4_wIVxTRe5QiRYwEFoUbV1V9bFtu5FLal0vZnLaWwg5tA6enhzBpxJNdrS0v1RcPpyeNP-9r3cUDGmeftwz9v95UQ","e":"AQAB"}`
		t.Setenv("DECK_JWK_KEY", jwkKey2)
