...
func TestOauth2CredentialUpdate(T *testing.T) {
	...
	oauth2Cred, err = client.Oauth2Credentials.Get(defaultCtx, consumer.ID, oauth2Cred.ID)
	assert.Nil(err)
...
func TestOauth2CredentialDelete(T *testing.T) {
	...
	err = client.Oauth2Credentials.Delete(defaultCtx, consumer.ID, oauth2Cred.ClientID)
	assert.Nil(err)

	oauth2Cred, err = client.Oauth2Credentials.Get(defaultCtx, consumer.ID, oauth2Cred.ClientID)
	assert.NotNil(err)
	assert.Nil(oauth2Cred)
...
func TestOauth2CredentialListMethods(T *testing.T) {
	...
	oauth2CredsForConsumer, next, err := client.Oauth2Credentials.ListForConsumer(defaultCtx,
		consumer1.ID, nil)
	assert.Nil(err)
	assert.Nil(next)
